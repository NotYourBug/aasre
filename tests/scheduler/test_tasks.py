...
class TestMessageBuilders:
    @pytest.mark.parametrize(
        "kind",
        [
            TaskKind.DAILY_SUMMARY,
            TaskKind.WEEKLY_AUDIT,
            TaskKind.INCIDENT_WINDOW_REPLAY,
            TaskKind.SYNTHETIC_RUN,
            TaskKind.CUSTOM_INVESTIGATION,
        ],
    )
    def test_investigation_backed_tasks_prefer_canonical_markdown(
        self,
        kind: TaskKind,
    ) -> None:
        task = ScheduledTask(
            id=f"canonical-{kind.value}",
            kind=kind,
            cron="0 9 * * *",
        ...
        runners = SchedulerRunners(
            agent=lambda _payload: "agent output",
            investigation=lambda _payload: {
                "report_markdown": "# Canonical",
                "report": "*Slack legacy*",
            },
        )

        assert tasks_mod.build_message(task, runners) == "# Canonical"

    @pytest.mark.parametrize(
        ("kind", "title"),
        [
            (TaskKind.DAILY_SUMMARY, "Daily Reliability Summary"),
            (TaskKind.WEEKLY_AUDIT, "Weekly Alert Audit"),
            (TaskKind.INCIDENT_WINDOW_REPLAY, "Incident Window Replay"),
            (TaskKind.SYNTHETIC_RUN, "Synthetic Test Summary"),
            (TaskKind.CUSTOM_INVESTIGATION, "Custom Investigation"),
        ],
    )
    def test_fixed_fallbacks_are_canonical_markdown(
        self,
        kind: TaskKind,
        title: str,
    ) -> None:
        task = ScheduledTask(
            id=f"fallback-{kind.value}",
            kind=kind,
            cron="0 9 * * *",
            provider=Provider.FEISHU,
            chat_id="oc_target",
        ...
        message = tasks_mod.build_message(task, runners)

        assert f"**{title}**" in message
        assert "_Generated by OpenSRE scheduled delivery_" in message
