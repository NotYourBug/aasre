...
这解释了当前飞书链路的真实缺陷：现有实现**只发 `msg_type="text"`**（全仓库仅两个发送点：
`gateway/transports/feishu/turn_output.py:37`、`integrations/feishu/delivery.py:47`，
**不存在 `post`/`md` 路径**。内容侧确实经过 `strip_html(...)`，见
`integrations/feishu/scheduled_delivery.py:50`）。纯 `text` **不渲染任何标记** ——
表格、围栏代码块、加粗、链接全部以原文显示，再叠加
`truncate(..., MAX_MESSAGE_SIZE=4096)` 的截断。含表格的调查报告因此发生**内容丢失**，
不只是渲染难看。

> **2026-09-15 更正**：本节原写「现有实现走 `post`/`md` 消息类型」，与代码不符 ——
> `msg_type(` 在两个发送点均为 `"text"`。**结论（内容丢失的定性）不变，路径更正。**
...
- **R30（用户，2026-09-13）：富文本走 `schema 2.0` + `tag: "markdown"`。** 见 §2.5。
  当前出站只发 `msg_type="text"`，**不渲染任何标记**（表格、代码块、链接全部以原文显示），
  再叠加 `truncate(..., 4096)` 的截断，属内容丢失，必须修。见 §2.5 的 2026-09-15 更正。
...
| **S2** | 感知层：附件与图片 | 下载+内联、图片 vision、停止丢弃非文本 | 无（需飞书 `im:message` / `im:message:readonly` 权限；**不是** `im:resource`，那是上传接口） | 发截图/日志文件，agent 能看到内容 |
...
30KB 时关闭流式、按字节预算**另发卡片**承载剩余；流式错误码则退回一次性完整**卡片**。
...
> **分页规则（级 2/3 共用）**：按**字节预算**与**表格数**双维度切分 —— 单卡片上限 30KB；
...
| 出站走纯 `msg_type="text"`：不渲染表格/代码块，且 `truncate(..., 4096)` 截断 | 高 | §2.5 已定性为**内容丢失**（非渲染问题；归因已于 2026-09-15 更正为 `text`，原写 `post`/`md`）；S6 必须把 `msg_type` 换成卡片，否则报告表格持续丢失 |
