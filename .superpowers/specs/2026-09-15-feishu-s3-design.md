...
> **客户端渲染已确认（2026-09-16）**：探针只能证明**服务端不拒绝**；客户端表现
> 由人工在真实飞书里核对，64 KiB 卡片发送与显示均正常（手机端与桌面端）。
> 若日后渲染出问题，`FEISHU_CARD_BUDGET_BYTES` 是回到 28KB 的唯一开关。
>
> **同一轮真机验证还抓到一个一级缺陷**：`update()` 在会话关闭后直接返回，预算
> 触发之后到达的文本被**全部丢弃**（实测 33,689 字符只投递 22,071）。12 条 stream
> 测试全都是「一次性喂完整文本」——恰好是触发后没有后续文本的唯一形状，因此
> 结构性地测不到。修法见下。

### 级 2 的续投契约（真机验证后补充）

级 2 **不在预算触发的那一刻结束**。真实 turn 是边生成边喂的，触发点之后到达的
才是答案的大部分，必须继续投递。会话因此维护一个**投递游标** `_delivered`：

- 游标只在**整批卡片全部落地后**推进；某张卡失败则游标不动，会话停机而非跳过
  空洞 —— 否则后续切片会略过从未发出的页。
- 新文本攒够一个预算才发一批，续卡因此保持整页大小，而不是每个 delta 一张卡。
- `finish()` 负责冲刷不足一个预算的尾部，否则最后几 KB 会滞留。
- 正常关闭路径也要推进游标（流式卡上已有的内容即已投递），否则干净关闭会被
  误判为未投递而重发全文。
