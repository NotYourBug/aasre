...
| `--chat-id` | Cond. | Required for telegram/discord/rocketchat/feishu. Optional for slack when an incoming webhook or `SLACK_DEFAULT_CHAT_ID` is configured, and not needed for `interactive_shell`. |
