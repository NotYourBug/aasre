...
- **R27（用户，2026-09-12）：卡片审批完全取代文本审批。** S3 落地后移除 R15 的文本回复
  approve/deny 路径，不保留共存分支。
...
| **S3** | 审批卡片化（升级 R15） | 卡片构建 + `card.action.trigger` + 3s ack + 原地更新 + 取代文本路径 | 无代码前置；**需管理员先完成飞书控制台回调配置**（见 §2.2、§7） | 按钮审批可用；群聊旁观者无法误触 |
| **S4** | 对话体验 | 占位符/流式预览、长消息分片、outbound 回复线程 | 无 | 长调查可见进度；超长回答不截断 |
| **S5** | 告警与投递卡片化 | watchdog 告警卡片、报告/定时投递卡片、反馈按钮 | S3（复用卡片基建） | 告警/报告在飞书里是卡片而非纯文本 |
| **S6** | agent 工具面 | `integrations/feishu/tools/`（按需裁剪，非照搬 slack 9 个） | S1（凭据层） | agent 能主动发飞书消息 |

**硬依赖只有两条**：S6 需要 S1 的凭据层；S5 需要 S3 的卡片基建。其余彼此独立，可并行。
...
| 消息反应（reactions） | slack / discord | **备选**：SDK 已确认支持，若 S3/S5 需要进度状态再纳入 |
...
| 卡片路径需控制台手工配置（订阅回调 + 重新发布） | 中 | **代码侧不可自动化**；S3 开工前需用户先完成配置，否则按钮点击报 `200340` |
| S2 需飞书权限 `im:resource` | 中 | 同上，需控制台授权 |

## 8. 后续

本 spec 落地后，S1 走 `writing-plans` 出实现计划。S2–S6 各自在本 spec 的框架下单独 brainstorm
