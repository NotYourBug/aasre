...
- Completion notifications can go to email ([SMTP](/smtp)), Telegram
  ...
  ([Buzz](/messaging/buzz)), Feishu, or any combination of those
...
| `/background notify set <channel[,channel...]>` | Set completion channels — `email`, `telegram`, `rocketchat`, `buzz`, `feishu`, or a comma-separated list |
...
```text
/background notify set telegram    # or: email, rocketchat, buzz, feishu, or email,telegram
...
Those stats are included in notification messages (email, Telegram, Rocket.Chat, Buzz, Feishu)
...
| `feishu` | Feishu alert-push app | Set `ALERTPUSH_APP_ID`, `ALERTPUSH_APP_SECRET`, `FEISHU_ALARM_RECEIVE_ID` |

All channels send the same summary structure: **Root cause**, **Top analysis**,
**What to do next**, plus a short internal stats section. Feishu sends the
complete summary as automatically paginated Markdown cards and falls back to
ordered plain-text chunks without truncating content. Telegram, Rocket.Chat,
and Buzz messages are plain text and are capped at the 4,096-character limit
(sections may be shortened to fit). Email can carry a fuller version of the
same content.
...
| `feishu` | Uses `--chat-id` as a `chat_id`, or the configured default Feishu destination |
