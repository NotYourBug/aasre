...
| `/background notify set &lt;channel[,channel...]&gt;` | Set channels — `email`, `telegram`, `rocketchat`, `buzz`, `feishu`, or a combination |
...
Channels are **off by default**. They are cleared when the REPL process exits; `/new` keeps notify prefs but clears jobs and turns background mode off. `email` requires [`smtp`](/smtp); `telegram` requires [`telegram`](/messaging/telegram); `rocketchat` requires [`rocketchat`](/messaging/rocketchat); `buzz` requires [`buzz`](/messaging/buzz); `feishu` requires `ALERTPUSH_APP_ID`, `ALERTPUSH_APP_SECRET`, and `FEISHU_ALARM_RECEIVE_ID`. Interactive shell only — there is no `opensre background …` CLI command. Full guide: [Background investigations](/background-investigations).
...
### Watchdog
Monitor a local process and send **Telegram** alarms when CPU, memory, or runtime thresholds breach. Configure Telegram credentials before use (via `/integrations setup telegram` or env — see messaging docs).
...
First-run setup: LLM keys, integrations, Telegram, alert listener. A blank LLM key defers provider setup instead of blocking the wizard. Requires exclusive stdin — the REPL tears down prompt_toolkit before the wizard runs.
...
If you skip onboarding's LLM key, later turns that need a model point you at `/auth login` instead of a raw provider error. On first launch with no scheduled loops, the shell offers three starter loops — Escape skips. See [Scheduled deliveries](/cron).
